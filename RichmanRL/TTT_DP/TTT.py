...
class BoardVisualizer():
    ...
    def draw_mark(self,row, col):
        ...
        if self.board[3*row+col] == 1:
            pygame.draw.line(self.screen, self.BLACK, (x - self.SQUARE_SIZE // 4, y - self.SQUARE_SIZE // 4), 
                            (x + self.SQUARE_SIZE // 4, y + self.SQUARE_SIZE // 4), self.LINE_WIDTH)
            pygame.draw.line(self.screen, self.BLACK, (x + self.SQUARE_SIZE // 4, y - self.SQUARE_SIZE // 4), 
                            (x - self.SQUARE_SIZE // 4, y + self.SQUARE_SIZE // 4), self.LINE_WIDTH)
        elif self.board[3*row+col] == 2:
            pygame.draw.circle(self.screen, self.BLACK, (x, y), self.SQUARE_SIZE // 4, self.LINE_WIDTH)

...
class BiddingHexNNPolicy(Policy):
    """Approximation of a bidding policy with a neural network."""

    def __init__(
        self, input_dimension, output_dimension: int, alpha: float, verbose=True
    ):
        """Constructor.
        ...
        Args:
            ...
            verbose: Whether or not to enable logging statements
        """
        self.logger = logging.getLogger("BiddingHexNNPolicy")
        if verbose:
            self.logger.setLevel(logging.DEBUG)
        else:
            self.logger.setLevel(logging.ERROR)
        ...
        self.mlp = ConvNet(input_dimension, output_dimension,)
    ...
    def update(
        ...
        probs = self.mlp(bidding_feature, input_feature, legal_mask, return_prob=True)
    ...
    def __call__(self, state: RichmanObservation) -> int:
        ...
        action_probs = self.mlp(bidding_feature, state_feature, legal_mask, return_prob=True)

...
class Board():
    ...
    def nodeToInt(self, row, column):
        return self.size*row + column
    
    def getNeighbors(self, row, column):
        potentialNeighbors = [(row, column-1), (row, column+1),
    ...
    def win(self, player):
        if(player == self.PLAYER1):
            return self.dsu.find(self.size*self.size) == self.dsu.find(self.size*self.size+1)
        elif(player == self.PLAYER2):
            return self.dsu.find(self.size*self.size+2) == self.dsu.find(self.size*self.size+3)
...
class Hex(AECEnv):
    ...
    def __init__(
        self, board_size = 11, render_mode: str | None = None):
        super().__init__()
            ...
            i: spaces.Dict(
                {
                    ...
                    "action_mask": spaces.Box(low=0, high=1, shape=(board_size**2,), dtype=np.int8),
                }
            )
        ...
        self.infos = {i: {"legal_moves": list(range(0, board_size**2))} for i in self.agents}
    ...
    def observe(self, agent):
        ...
        legal_moves = np.array(self.board.legal) if agent == self.agent_selection else np.zeros(self.board_size**2)
